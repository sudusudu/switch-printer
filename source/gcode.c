...
static void print_thread_func(void *arg) {
    ...
    if (!fp) { g_status.state = PRINTER_ERROR; return; }
    int total_lines = 0;
    ...
    while (fgets(line_buf, sizeof(line_buf), fp) && !g_cancel) {
        while (g_paused && !g_cancel) svcSleepThread(100000000ULL);
        if (g_cancel) break;
...
Result gcode_start_print(Ch340Device *dev, const char *file_path) {
    ...
    if (g_status.state == PRINTER_PRINTING) return MAKERESULT(Module_Libnx, LibnxError_Busy);
    g_dev = dev;
    ...
    if (R_FAILED(rc)) { g_status.state = PRINTER_IDLE; return rc; }
    threadStart(&g_print_thread);
    return 0;
}

Result gcode_pause(void) { g_paused = true; g_status.state = PRINTER_PAUSED; return 0; }
Result gcode_resume(void) { g_paused = false; g_status.state = PRINTER_PRINTING; return 0; }
...
PrinterStatus *gcode_get_status(void) { return &g_status; }
