...
static void draw_ui(PrintConsole *console, Ch340Device *dev) {
    ...
    const char *state_str[] = {"[ OFFLINE ]", "[ IDLE ]", "[ PRINTING ]", "[ PAUSED ]", "[ ERROR!! ]"};
    const char *state_color[] = {"\x1b[31m", "\x1b[32m", "\x1b[33m", "\x1b[36m", "\x1b[31m"};
    int s = st->state;
    ...
    if (s == 2 || s == 3) {
        ...
        printf("   [");
        for (int i = 0; i < bar_w; i++) printf("%c", i < filled ? '=' : ' ');
        printf("]\n");
    }
    printf("\n   ===============================\n\n");
    printf("   WiFi: %s\n", httpd_get_ip());
